var IMAGESPATH = 'lib/dialog/images/'; 
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" style="background:url('+IMAGESPATH+'button.gif)  no-repeat;width:58px;height:24px;line-height:24px;cursor:pointer;overflow:hidden;border:0;" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" style="background:url('+IMAGESPATH+'button.gif)  no-repeat;width:58px;height:24px;margin-left:8px;cursor:pointer;line-height:24px;overflow:hidden;border:0;" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
    </tr>\
...
Dialog.alert = function (msg, func, Aclose, w, h) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
